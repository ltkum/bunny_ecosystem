...
async def assign_initial_channels_and_roles():
    for gd in ['TEST_SERVER']:  # 'FFSFR_SERVER'
        ...
        RelevantRoles.setRoleFromGuild(guild, 'bo',
                                       int(os.getenv(f"{gd}_BO_ROLE_ID", "0")))
        RelevantRoles.setRoleFromGuild(
            guild, 'tracker', int(os.getenv(f"{gd}_TRACK_ROLE_ID", "0")))
        RelevantRoles.setRoleFromGuild(
            guild, 'commentator', int(os.getenv(f"{gd}_COMM_ROLE_ID", "0")))
...
async def on_ready():
    ...
    await RelevantChannels.getSpecificChannel(test_guild, "dashboard").send(
